...
echo Installing Book Author...

if not exist "%PAYLOAD_ZIP%" (
  echo Could not find the Book Author payload archive.
  pause
  exit /b 1
)

if exist "%INSTALL_DIR%" (
  echo Updating existing Book Author installation in "%INSTALL_DIR%".
) else (
  mkdir "%INSTALL_DIR%"
)
...
if errorlevel 1 (
  echo Book Author could not be extracted.
  pause
  exit /b 1
)
